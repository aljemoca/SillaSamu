...
class esp32Node(Node):
    def __init__(self):
        ...
        self.publisher_status = self.create_publisher(String, 'esp32_status',10)
        self.connect()
    ...
    def connect(self):
        ...
        try:
            self.esp32 = ComESP32.ComESP32(self.port)
            self.connected = True
            msg.data = "CONNECTED"
        ...
        self.get_logger().info(f'ESP32 : {msg.data}')
...
#########################################################################################################
    def timer_callback(self):
        #Agregado para robustecer conexiones
        try:
            if self.connected == True:# Añado esta verificación para que no se rompa el nodo
                datos = self.esp32.obtenerMedidas()
                print(datos)
                #self.get_logger().info(f'Datos Recibidos: {datos}')
            if not datos[0]:
                msg = Int32()
                msg.data = datos[1]
