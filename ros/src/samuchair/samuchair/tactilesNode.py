...
class tactilesNode(Node):
    ...
    def timer_callback(self):
        msg =Int32()
        #msgD = Int32()
        #Poner aquí la lectura d elos táctiles. Se utiliza un muestreo de 5 señales
        self.readLeft = self.tactil_izq.digitalRead()
        
        if self.readLeft == self.lastreadLeft:
            self.leftCount += 1
        else:
            self.leftCount = 0
            self.lastreadLeft = self.readLeft
        if self.leftCount >= 3:
            self.leftCount = 0
            if self.readLeft != self.leftStatus:
                self.leftStatus = self.readLeft
                msg.data = self.readLeft
                self.publisher_tactil_izq.publish(msg)
            
        self.readRight = self.tactil_der.digitalRead()
        if self.readRight == self.lastreadRight:
            self.rightCount += 1
        else:
            self.rightCount = 0
            self.lastreadRight = self.readRight
        if self.rightCount >= 5:
            self.rightCount = 0
            if self.readRight != self.rightStatus:
                self.rightStatus = self.readRight
                msg.data =  self.readRight
