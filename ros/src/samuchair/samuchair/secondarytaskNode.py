...
class SecondaryTask:
    def __init__(self):
        ...
        try:
        ###Código para enumerar los dispositivos disponibles
            for i, dev in enumerate(sd.query_devices()):
                if 'GO Work USB' in dev['name']:
                    mic_id = i
                    break
        #####################################################
    ...
    def maxFrame1(self, indata,  frames, time, status):
        # Calcular el valor RMS del bloque de audio actual
        rms_data = np.sqrt(np.mean(indata**2))

        if self.flanco == 0:
            # El umbral RMS suele ser más bajo que el del pico máximo (ej. 0.02)
            if rms_data > 0.02:
                self.t1 = ti.time() 
                self.semaforo = True
                self.flanco = 1
                self.tiempo = self.t1 - self.t0 +self.contframes/self.Fs
            else:
    ...
    def maxFrame(self, indata, frames, time, status):
        ...
        if self.flanco==0:
            if  data >0.05:   #Antiguo valor 0.05
                self.t1 = ti.time() 
                self.semaforo=True
...
class secondarytaskNode(Node):
    def __init__(self):
        ...
        self.publisher_status = self.create_publisher(String, 'Headphones_status',10)
        ...
        #self.timerStatus = self.create_timer(5, self.timer_statusConnection)
    ...
    def initConnection (self):
        ...
        try:
            ...
            self.publisher_status.publish(msg)
        self.get_logger().info(f'AURICULARES : {msg.data}')

    def timer_statusConnection(self):
    ...
    def timer_lectura(self):
        ...
        if self.st.dataReady() and self.timeout < 4.0:
            ...
            if self.activate:
                ...
                #msg1.data = self.st.readStatus()
                #self.get_logger().info(f'Status: {msg1.data}')
